...
public final class String implements CharSequence {
    ...
    public void getChars(int srcBegin, int srcEnd, char[] dst, int dstBegin) {
        for (int i=srcBegin; i<srcEnd; i++) dst[i - srcBegin + dstBegin] = charAt(i);
    }
    ...
    private static String leftPadding(int width, int number) {
        ...
        for (int i=0; i<width-s.length(); i++) s = "0" + s;
        return sign + s;
    }
    ...
    public static String format(String format, Object... args) {
        ...
        for (int i=1; i<parts.length; i++) {
            ...
            if (chunk == null) throw new RuntimeException("Illegal format at: " + part);
            sb.append(chunk);
    ...
    public int hashCode() {
        int length = length();
        int code = 0;
        for (int i=0; i<length; i++) code = code * 31 + charAt(i);
        return code;
    }
